...
> **Navigation**: [← Documentation Index](INDEX.md) | [System Architecture](system-architecture.md) | [Domain Services Reference](domain-services.md) | [Granular Codebase Technical Manual](knowledgebase/codebase-technical-manual.md)
...
function fHideAllTooltips() {
  ...
  [...popoverTriggerList].map((popoverEl) => Popover.getOrCreateInstance(popoverEl).hide());
  const tooltipTriggerList = document.querySelectorAll('[data-bs-toggle="tooltip"]');
  [...tooltipTriggerList].map((tooltipEl) => Tooltip.getOrCreateInstance(tooltipEl).hide());
...
The Webpack bundler executes using `webpack.prod.js` and outputs production assets to `build/FoE-Info_WEBSTORE/`.
