# FoE-Info-Extension Architecture & Extension Lifecycle Guide
...
| `unlimitedStorage`                    | Permission       | Removes the standard 5MB storage quota limit for `chrome.storage.local`, enabling large-scale local caching of static entity definitions, player visit histories, and Great Building donor datasets.                 |
