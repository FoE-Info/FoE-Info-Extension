...
| **Live HAR captures**        | `docs/har/` (git-ignored) | 44 Chrome DevTools `.har` recordings (2.3 GB) used as ground truth. Extracted into sibling `../metadata-store/extracts/` with `npm run metadata:extract-hars` (49 MB, 103 RPCs, 27 bundles, 13 cities) and indexed into the metadata graph; never committed. |
