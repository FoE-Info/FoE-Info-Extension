...
- [ ] Legacy Bridge Modernization (Slice 3): Decommission `cityRoutes.js` by adding `.register(dispatcher)` to relevant city services (`CityProductionService.js` / `CityMapService`), wiring into `registerServices.js`, and deleting `cityRoutes.js`.
