...
- [ ] Legacy Bridge Modernization (Slice 5): Decommission `combatRoutes.js` by adding `.register(dispatcher)` to `GuildBattlegroundService.js`, `GuildExpeditionService.js`, and `ArmyUnitManagementService.js`, unhooking from `legacyBridge.js`, and deleting `combatRoutes.js`.
