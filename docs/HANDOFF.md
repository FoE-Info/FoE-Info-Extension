...
Updated 2026-09-12 after the third-pass FoE expert audit (metadata graph + HAR payloads edge-to-edge), the modern-web-guidance Tier 1/2 remediation, and the quad-graph exploration suite.
...
- **Modern-Web Guidance Expert & Skill Overlay**:
  ...
  - **Verification**: `node --test tests/agents/agent-config.test.mjs` 13/13 (links, budgets, obsolete-path guard) and `npm run verify` exit 0 — 1,033 tests / 98 suites, prettier clean, 0 eslint errors, i18n 100%, dev build compiles. Canonical counts unchanged (36 agents / 17 rules / 53 skills). **Uncommitted.**
