...
const { extractEntityBoosts: extractBoosts } = require('./boostExtractor.js');
...
function formatMilitaryBoosts(rawBoosts) {
  ...
  logger?.debug('Formatted military boosts matrix:', {
    redBase: `${redBaseAtt.toString()}% Att / ${redBaseDef.toString()}% Def`,
    blueBase: `${blueBaseAtt.toString()}% Att / ${blueBaseDef.toString()}% Def`,
  });
