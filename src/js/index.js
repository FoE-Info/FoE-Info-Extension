...
function handleRequestFinished(request) {
  ...
  // if (contentType == "json") {
  if (
    request.request.url.match(/https:\/\/.*\.forgeofempires\.com\/game\/json\?h=/g) ||
    request.request.url.match(/https:\/\/foe.*\.innogamescdn\.com\/start\/metadata\?id=(.*)/g)
  ) {
                        ...
                        GuildDonations.forEach((member) => {
                          ...
                          if (member[1] == entry.player.name) {
                            if (
                              ...
                              entry.action.toLowerCase() == 'guild expedition: difficulty unlocked'
                            )
