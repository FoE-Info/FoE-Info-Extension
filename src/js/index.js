...
async function processContentDirect(
  ...
  // Direct handling for metadata?id=* responses from CDN
  if (
    (reqUrl.includes('metadata?id=') || reqUrl.includes('metadata')) &&
    !reqUrl.includes('/game/json')
  ) {
    try {
      const entries = Array.isArray(parsed) ? parsed : [parsed];
      entries.forEach((item) => {
        if (item) processMetadataEntry(item);
      });
      saveCityEntityDefs();
      metadataLoaded = true;
      if (pendingStartupMsg) {
        console.debug(
          'Flushing pendingStartupMsg from direct metadata listener',
        );
        startupService(pendingStartupMsg);
        pendingStartupMsg = null;
      }
    } catch (e) {
      console.error('Error processing direct metadata payload', e);
    }
  }
      ...
      if (
        ...
        const requests = (msg.responseData || []).map((item) => {
          ...
          return fetch(item.url)
            ...
            .catch((err) => {
              console.error('Failed loading metadata item:', item.url, err);
              return null;
            });
        });
        ...
        if (msg.requestMethod == 'getResourceDefinitions') {
          ...
          msg.requestMethod == 'getResourceBag' ||
          ...
          items.forEach((item) => {
            ...
            if (
              bagType === 'ClanMain' ||
              (typeof bagType === 'string' &&
                (bagType.toLowerCase().includes('clan') ||
                  bagType.toLowerCase().includes('treasury')))
            ) {
              processTreasuryData(resources);
            } else if (bagType === 'PlayerMain') {
              getPlayerResources({ responseData: { resources } });
            }
          });
        }
        ...
        const hostHeader = (headers || []).find(
          (header) =>
            header.name &&
            (header.name.toLowerCase() === ':authority' ||
              header.name.toLowerCase() === 'host'),
        );
        if (hostHeader && hostHeader.value) {
          setGameOrigin(hostHeader.value.split('.')[0]);
        } else if (reqUrl) {
          try {
            const u = new URL(reqUrl);
            setGameOrigin(u.hostname.split('.')[0]);
          } catch (e) {
            console.error('Failed to parse GameOrigin from URL', e);
          }
        }
        ...
        const fetchStorage =
          storageBackend ? storageBackend.get(null) : Promise.resolve({});
        ...
        fetchStorage
          ...
          .catch((err) => {
            console.error('Storage get fallback:', err);
            receiveStorage({});
            startupService(msg);
          });
          ...
          try {
            const postDataText =
              (
                ...
                request.request.postData.text
              : '[]';
            postDataObj = JSON.parse(postDataText);
          } catch (e) {}
          getConstructionRanking(msg, postDataObj);
        ...
        try {
          const postDataText =
            (
              ...
              request.request.postData.text
            : '[]';
          payload = (JSON.parse(postDataText)[0] || {}).requestData || {};
        } catch (e) {}
          ...
          if (
            ...
            members.forEach((entry) => {
              const memberRow = [
                ...
                0, // Goods Spent GBG
                0, // Goods Spent GE
                0, // Goods Donated Building
                0, // Goods Donated ???
                0, // Goods Donated
              ];
              for (let a = 0; a < helper.numAges; a++) {
                memberRow.push(0);
              }
              GuildDonations.push(memberRow);
	  ...
	  <table id="friendsText2"><tr><th>Name</th><th>Title</th><th>ID</th><th>Era</th><th>Battles</th><th>Score</th></tr>`;
          ...
          if (showOptions.showContributions || showOptions.showLogs) {
                ...
                GuildDonations.forEach((member) => {
                  ...
                  if (member[1] == entry.player.name) {
                    if (
                      ...
                      member[9] += entry.amount; // manual donation
                    } else {
                      console.debug(entry.action, entry.amount);
                    }
            ...
            if (showOptions.showContributions) {
              ...
              let eraHeaders = '';
              for (let a = 0; a < helper.numAges; a++) {
                eraHeaders += `<th>${helper.fGVGagesname(
                  helper.fAgefromLevel(helper.numAges - a),
                )}</th>`;
              }
			...
			<table id="treasurytable" class="overflow table collapse show"><tr><th>Name</th><th>Medals Spent</th><th>Medals Returned</th><th>Medals Donated</th><th>Medals Total</th><th>Goods Spent GBG</th><th>Goods Spent GE</th><th>Goods Donated Building</th><th>Goods Donated ???</th><th>Goods Donated</th>${eraHeaders}</tr>`;
              GuildDonations.forEach((member) => {
                // rank,name,medals: spent,returned,donated, goods: spent,returned,donated
                if (member[0] != MyInfo.guild) {
                  let eraCells = '';
                  for (let a = 0; a < helper.numAges; a++) {
                    eraCells += `<td>${member[10 + a] ?? 0}</td>`;
                  }
                  treasuryHTML += `<tr><td>${member[1]}</td><td>${
                    member[2]
                  }</td><td>${member[3]}</td><td>${
                    member[4]
                  }</td><td>${member[2] + member[3] + member[4]}</td><td>${
                    member[5]
                  }</td><td>${member[6]}</td><td>${member[7]}</td><td>${
                    member[8]
                  }</td><td>${member[9]}</td>${eraCells}</tr>`;
                }
              });

              if (GuildTreasury) {
                treasuryHTML += `<tr></tr>`;
                treasuryHTML += `<tr><th>Era:Resource</th><th>Treasury</th><th>Donations</th><th>GE Cost</th><th>GBG Cost</th><th>Net Change</th></tr>`;
                GuildTreasury.forEach((rss) => {
                  treasuryHTML += `<tr><td>${rss[1]}:${rss[2]}</td><td>${rss[3]}</td><td>${rss[4]}</td><td>${rss[5]}</td><td>${rss[6]}</td><td>${rss[7]}</td></tr>`;
                });
              }
...
function safeProcessContent(request, processContent) {
  try {
    ...
    const res = request.getContent((content, encoding) => {
...
export function processTreasuryData(resources) {
  ...
  if (showOptions.showTreasury) {
    ...
    }px" class="overflow resize collapse ${collapse.collapseTreasury ? '' : 'show'}"><table id="treasurytable">`;
