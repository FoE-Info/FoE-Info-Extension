  ...
  if (isGameJson || isMetadata) {
    ...
    request
      ...
      .then(async (result) => {
        if (!result) return;
        const [body, encoding] =
          Array.isArray(result) ? result : [result, null];
        if (!body) return;
            ...
            if (
                ...
                if (msg.responseData.length) {
                  ...
                  for (var j = 0; j < msg.responseData.length; j++) {
                    ...
                    if (selected.type == 'greatbuilding') {
                      ...
                      GBselected.total =
                        selected.state.forge_points_for_level_up;
                      // donor2HTML += GBselected.total + '\n';
                      if (selected.state.invested_forge_points)
                        GBselected.current =
                          selected.state.invested_forge_points;
                      else GBselected.current = 0;
                      levelText += `<table>`;
              ...
              if (
                ...
                for (var j = 0; j < msg.responseData.rankings.length; j++) {
                  if (
                    msg.responseData.rankings[j].player.hasOwnProperty(
                      'is_self',
                    )
                  ) {
                    if (
	  ...
	  <span data-i18n="reward"><strong>REWARDS:</strong></span></p>
              ...
              if (msg.requestMethod == 'getConstructionRanking') {
                ...
                msg.requestMethod == 'getAvailablePackageForgePoints'
              ) {
                /*ForgePoints*/
