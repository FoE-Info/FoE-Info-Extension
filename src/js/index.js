...
let areCityEntitiesLoaded = false;
chrome.runtime.onMessageExternal.addListener(function (request, sender, sendResponse) {
  console.log("Message", request);
  if (typeof request["foe_city_entities"] !== "undefined") {
    processAjaxRequest(request["foe_city_entities"], null);
    areCityEntitiesLoaded = true;
  }
});
...
function handleRequestFinished(request) {
  ...
  if (contentType == "json") {
    ...
    request
      ...
      .then(([body]) => processAjaxRequest(body, request));
  }
}

const processAjaxRequest = (body, request) => {
  ...
  if (parsed && parsed.length) {
    for (var i = 0; i < parsed.length; i++) {
      ...
      } else if (msg.requestClass == "StartupService" && msg.requestMethod == "getData" && request) {
	  ...
	  <span data-i18n="reward"><strong>REWARDS:</strong></span></p>
        ...
        /*GB Donors */
        if (msg.requestMethod == "getConstructionRanking" && request) {
          // console.debug('msg:', msg);
      ...
      } else if (msg.requestClass == "GuildBattlegroundSignalsService" && request) {
        // GuildBattleground
