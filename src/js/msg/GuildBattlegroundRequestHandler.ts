...
type GuildBattlegroundDeps = {
  showOptions: BattlegroundOptions;
  clearForBattleground: () => void;
  getLeaderboard: MessageHandler<BattlegroundMessage>;
  getPlayerLeaderboard: MessageHandler<BattlegroundMessage>;
  getBattleground: MessageHandler<BattlegroundMessage>;
  getState: MessageHandler<BattlegroundMessage>;
  getBuildings: MessageHandler<BattlegroundMessage>;
};

export function handleGuildBattlegroundRequest(
  msg: BattlegroundMessage,
  deps: GuildBattlegroundDeps,
): boolean {
  if (!msg) {
    return false;
  }
