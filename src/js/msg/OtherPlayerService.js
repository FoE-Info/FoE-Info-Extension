...
export function otherPlayerService(msg) {
  ...
  const origin = (
    GameOrigin && GameOrigin.trim() ?
      GameOrigin
    : 'en7').toLowerCase();
  const safePlayerName = helper.escapeHTML(PlayerName);
  const safeClanName =
    player.clan && player.clan.name ?
      helper.escapeHTML(player.clan.name)
    : 'NO GUILD';
  visitstatsHTML = `<div  role="alert">
	${element.close()}
	<p href="#visitstatsText" data-bs-toggle="collapse"><a href="https://foe.scoredb.io/${encodeURIComponent(origin)}/Player/${encodeURIComponent(PlayerID)}" target="_blank"><strong>${safePlayerName}</strong></a> (${safeClanName})</p>`;
  ...
  if (showOptions.showStats) {
    ...
    try {
      const calculatedStats = cityStatsCalculator.calculateCityStats({
        entities: map_entities || [],
        playerEra: msg.responseData?.other_player_era,
      });
      calculatedStats.clanGoods = clanGoods;
      renderCityStats('visit', calculatedStats, {
        isOwnCity: false,
        name: PlayerName,
        guild: player?.clan?.name,
        era:
          helper.fGVGagesname(msg.responseData?.other_player_era) ||
          msg.responseData?.other_player_era,
        score: player?.score,
        clanGoods: clanGoods,
        clanPower: clanPower,
      ...
      console.warn('Modern visitstats render error, falling back:', err);
      visitstats.innerHTML = visitstatsHTML + `</div></div>`;
      visitstats.className =
        'alert alert-dark alert-dismissible show collapsed';
    }
...
function getFriendsHTML(list) {
  ...
  list.forEach((entry) => {
    var html = '';
    const safeName = helper.escapeHTML(entry.name);
    // console.debug(entry);
    ...
    // else if(type == 'Friend' && entry.is_friend != true && entry.accepted != true){
      ...
      // html += `<tr><td>${safeName}</td><td>PENDING</td></tr>`;
      // console.debug(entry);
      ...
      } else html += `<tr><td>${safeName}</td></tr>`;
