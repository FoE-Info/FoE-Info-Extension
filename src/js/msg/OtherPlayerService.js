  ...
  if (msg.responseData.city_map.entities.length) {
    ...
    map_entities.forEach((mapID, id) => {
      ...
      if (CityEntityDefs[mapID.cityentity_id]) {
        ...
        var boost = {};
        const entityAge =
          mapID.level ? helper.fAgefromLevel(mapID.level) : msg.responseData.other_player_era;
            ...
            for (let o in opt) {
              ...
              for (let p in products) {
                ...
                let onlywhenmotivated =
                  products[p].onlyWhenMotivated && products[p].onlyWhenMotivated === true ?
                    true
                  : false;
  ...
  if (showOptions.showStats) {
    ...
    if (googleSheetAPI) {
      document
        ...
        .addEventListener('click', () => post_webstore.postPlayerToSS(visitData));
    }
  }
  // console.debug('showStats:',showOptions.showStats);
}
