    ...
    browser.storage.local
      ...
      .then((items) => {
        ...
        if (items[GameOrigin + 'BGtime'])
          setBGtime(
            new Date(items[GameOrigin + 'BGtime'] * 1000).toLocaleString(),
          );
...
export function getState(msg) {
  ...
  if (msg.responseData.stateId == 'subscribed') {
    ...
    msg.responseData.playerLeaderboardEntries.forEach((entry) => {
      ...
      battlegroundHTML += `<tr><td>${entry.rank}</td><td>${safePlayerName}</td><td>${wonNegotiations}</td><td>${wonBattles}</td><td>${attrition}</td></tr>`;
