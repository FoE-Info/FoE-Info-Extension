...
function buildOwnCityCard({
  ...
  const arcBonusText =
    spec.arcPercent && !spec.arcPercent.isZero() ?
      `Arc <span data-i18n="bonus">Bonus</span>: ${formatPercent(spec.arcPercent)}`
    : '';
  const cfBonusText =
    spec.chatBonus && !spec.chatBonus.isZero() ?
      `CF <span data-i18n="bonus">Bonus</span>: ${formatPercent(spec.chatBonus)} (${formatStatNumber(spec.goodsPerQuest)} <span data-i18n="goods">Goods</span>)`
    : '';
  let specBonusesHTML = '';
  if (arcBonusText && cfBonusText) {
    specBonusesHTML = `<div>${arcBonusText}, ${cfBonusText}</div>`;
  } else if (arcBonusText) {
    specBonusesHTML = `<div>${arcBonusText}</div>`;
  } else if (cfBonusText) {
    specBonusesHTML = `<div>${cfBonusText}</div>`;
  }
  ...
  <div class="d-flex align-items-center justify-content-between mb-1">
    <div class="d-flex align-items-center gap-1 text-truncate">
      <span role="button" class="foe-collapse-icon me-1 flex-shrink-0" data-bs-toggle="collapse" href="#${prefix}Text"
        aria-expanded="${!isCollapsed}" aria-controls="${prefix}Text" title="Toggle Stats" data-i18n-title="toggle_stats">[-]</span>
