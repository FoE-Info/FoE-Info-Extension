...
function gbTabSafe(...args) {
  ...
  return `<div class="card ${cfg.darkMode === 'dark' ? 'text-light bg-dark' : 'text-dark bg-light'} alert show collapsed p-0">
    <div class="card-header fw-bold"><span data-i18n="gb">GB</span> <span data-i18n="donation">donation</span>${disconnected}${inactive}${locked}${element.close()}${element.copy('donationCopyID', 'info', 'right', collapse.collapseDonation)}</div><div class="card-body alert-success p-2">
...
function gbTabNotSafe(...args) {
  ...
  return `<div class="card ${cfg.darkMode === 'dark' ? 'text-light bg-dark' : 'text-dark bg-light'} ${alertClass} show collapsed p-0 ">
    <div class="card-header fw-bold"><span data-i18n="gb">GB</span> <span data-i18n="donation">Donation</span>${element.close()}${element.copy('donationCopyID', 'info', 'right', collapse.collapseDonation)}</div><div class="card-body ${alertClass} p-2">
...
function gbTabEmpty(...args) {
  ...
  return `<div class="card ${cfg.darkMode === 'dark' ? 'text-light bg-dark' : 'text-dark bg-light'} alert show collapsed p-0 ">
    <div class="card-header fw-bold"> GB Donation [${getPlayerLink(cfg.playerName, cfg.playerId)}]${element.close()}</div>
