...
function formatSurprisingSection(connections) {
  const lines = [
    ...
    '',
    '_Repository policy: Graphify’s standard surprise ranking is applied only to nodes sourced from the extension implementation under `src/`. Documentation, build tooling, and other repository metadata remain queryable in the graph but are excluded here._',
    '',
  ];
...
function formatRuntimeReport(graph) {
  ...
  return {
    ...
    report: [
      ...
      '',
      '_This report is derived only from code nodes under `src/`. The full graph remains unchanged and keeps documentation, build tooling, and repository metadata available for queries._',
      '',
      '## Scope',
      '',
      '- Included: executable extension code and runtime configuration under `src/`.',
      '- Excluded from reporting: documentation, build tooling, repository metadata, and non-code assets.',
