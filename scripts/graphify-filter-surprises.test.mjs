...
test('regenerates the architecture report from runtime source code only', async () => {
  ...
  const graph = {
    nodes: [
      ...
      },
      {
        ...
        file_type: 'document',
        source_file: 'docs/rules/workflow.md',
        community: 4,
      },
      {
        id: 'logo',
    ...
    hyperedges: [
      {
        ...
        nodes: ['request', 'documentation', 'rule-doc'],
        confidence: 'INFERRED',
      },
    ],
  };
  ...
  assert.doesNotMatch(
    report,
    /DevTools Network Dispatch Architecture|Workflow Rules|Mixed Documentation Group|Logo Asset/,
  );
...
test('limits surprising connections to the extension source tree', async () => {
    ...
    JSON.stringify({
      nodes: [
        ...
        },
        {
          ...
          source_file: 'docs/rules/workflow.md',
          community: 5,
          community_name: 'Rules Configuration',
        },
        {
          id: 'package-scripts',
          label: 'scripts',
  ...
  assert.doesNotMatch(
    report,
    /Documentation Catalog|Workflow Rules|package.json|`scripts`|Logo|stale documentation result/,
  );
...
test('limits suggested questions to source-tree nodes', async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      nodes: [
        ...
        { id: 'doc-rule', label: 'Workflow Rules', source_file: 'docs/rules/workflow.md' },
  ...
  await writeFile(
    ...
    [
      ...
      '- **What connects `Workflow Rules` to the application?**',
      '  _Rules configuration._',
  ...
  assert.doesNotMatch(report, /Workflow Rules|Architecture Guide|`scripts`|package\.json/);
