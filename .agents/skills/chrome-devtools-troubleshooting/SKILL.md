...
- Pass `--browser-url=http://127.0.0.1:9222` instead of `--autoConnect` (e.g. connecting to the isolated `foe-browser` instance).
- Enable remote debugging in Chrome (`chrome://inspect/#remote-debugging`) and accept the connection prompt if not using the pre-configured browser wrapper.
- Add `--logFile <absolute_path_to_log_file>` to capture debug logs for analysis.
- Increase `timeout` in the hook or MCP config if experiencing slow cold starts on resource-constrained machines.
...
### Step 6: Check Offline Guide and GitHub Issues

First consult the local offline reference [`references/troubleshooting.md`](references/troubleshooting.md). If it does not cover the specific error, check if the `gh` (GitHub CLI) tool is available in the environment:
