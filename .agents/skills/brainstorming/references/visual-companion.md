...
**Launching the server on Google Antigravity:**

```bash
# Launch using run_command with a short wait or background flag:
.agents/skills/brainstorming/scripts/start-server.sh --project-dir . --open
```

On Google Antigravity, background execution is handled natively by `run_command` (which moves long-running processes to the background if `WaitMsBeforeAsync` elapses) and managed via `manage_task` (`list`, `status`, `send_input`, `kill`).

Read `$STATE_DIR/server-info` or terminal output to verify the URL and port.
