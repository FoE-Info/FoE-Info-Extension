...
**Browser lifecycle in this workspace**: MCP config (`.agents/mcp_config.json` / `opencode.json`) launches `chrome-devtools-mcp` directly with `--browserUrl=http://127.0.0.1:9222`, only ever attaching to the already-running `foe-browser` instance; it never spawns a browser itself. If no browser is on port 9222, launch it via `foe-browser`. Direct upstream launch behavior can differ; inspect the MCP config before applying generic setup examples.
