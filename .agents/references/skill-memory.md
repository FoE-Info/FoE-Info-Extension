...
- `--signal` records the _grounded_ check you actually ran (a test command, a
...
3. **Curator** — `skill-memory.mjs log --outcome fail --lesson '<rule>'`
   records the evidence and idempotently promotes the rule into the canonical
   skill or flat subagent definition. `project-curator.mjs` audits usage,
   promotion gaps, staleness, and exact duplicates across the shared library.

A lesson changes future behavior only after it reaches the canonical definition.
`log --lesson` normally performs both operations; if promotion was interrupted,
run `skill-memory.mjs sync --skill <name>` to repair the gap.
