...
The workspace maintains a roster of 31 specialized domain subagents (`.agents/agents/*.md`). The primary agent must act as an orchestrator and tech lead, proactively delegating tasks matching their domain expertise via `invoke_subagent` rather than executing everything in a single thread.
...
4. **Knowledge Graphs & Architecture (3)**:
...
- Before executing code modifications or deep investigations, check whether the task fits any of the 31 specialized subagents in the directory above (e.g. Bootstrap UI, Great Buildings math, RPC network handlers, CDP browser testing).
...
1. **No Subagent Fits**: The task, script, or investigation falls outside the defined roles of the 31 specialists.
