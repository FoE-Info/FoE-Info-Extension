...
2. **Zero-Install & Multi-Ecosystem Python Tools**: For Python CLI tools (such as `graphify`), task runners and rules must use a 3-tier fallback chain (`uvx` -> `pipx run` -> direct binary) to support `mise`, `uv`, `pipx`, and standard `pip` setups seamlessly:
   `command -v uvx >/dev/null 2>&1 && uvx --from "graphifyy[gemini,mcp]==0.9.36" graphify ... || (command -v pipx >/dev/null 2>&1 && pipx run --spec "graphifyy[gemini,mcp]==0.9.36" graphify ... || graphify ...)`
3. Keep canonical tool versions in `.mise.toml` and package specifications. Do not hardcode machine-specific install paths or duplicate different versions across task runners.
