...
2. Do **NOT** hardcode specific version numbers (e.g. `22.23.2`) in scripts or commands — `mise` reads `.mise.toml` automatically.
3. Do **NOT** run `which`, `export PATH`, or manual setup commands in chat — tools are pre-activated.
4. Project `.env` environment variables are loaded automatically by `mise` and `~/.antigravity_env.sh`.
