...
2. **Zero-Install Python Tools**: Prefer `uvx --from "package[extras]" command` for running Python CLI tools and MCP servers. No local `.venv`, pre-install hooks, or global `pip install` steps are required. `uvx` automatically handles caching and execution in all agent subshells.
