  ...
  if (isCodebaseSourceSearch(toolName, args)) {
    if (!isQueryStampFresh()) {
      return {
        decision: 'deny',
        reason:
          'MANDATORY: Graphify knowledge graph exists at graphify-out/foe-info/graph.json. You must consult Graphify first before performing broad codebase searches. Use call_mcp_tool on "graphify-foe-info" (or "graphify-metadata-store" / "graphify-forge-hammer") with one of the native MCP tools:\n' +
