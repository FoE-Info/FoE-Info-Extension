  ...
  .get('http://127.0.0.1:9222/json', (res) => {
    ...
    res.on('end', async () => {
      ...
      try {
        ...
        console.error(
          '[Error Inspector] Failed to parse CDP targets JSON:',
          e.message,
        );
        process.exitCode = 1;
        return;
      ...
      let selectedTargets = targets.filter((t) => {
        ...
        if (
          targetFilter === null &&
          t.url &&
          t.url.includes('devtools://devtools/bundled/devtools_app.html')
        ) {
          return false;
        }
        ...
        if (targetFilter) {
          return (
            (t.url && t.url.includes(targetFilter)) ||
            (t.title &&
              t.title.toLowerCase().includes(targetFilter.toLowerCase()))
          );
        }
        return true;
      });
      ...
      console.log(
        `[Error Inspector] Monitoring ${selectedTargets.length} target(s) for ${durationMs}ms...`,
      );
      ...
      selectedTargets.forEach((target) => {
        const targetName =
          target.url ?
            target.url.includes('panel.html') ?
              'panel.html'
        ...
        ws.addEventListener('message', ({ data: msgStr }) => {
          try {
            ...
            if (msg.method === 'Runtime.exceptionThrown') {
              ...
              const argsText = msg.params.args
                .map((a) => a.value || a.description || JSON.stringify(a))
                .join(' ');
              const stack =
                msg.params.stackTrace ?
                  msg.params.stackTrace.callFrames
                    .map(
                      (f) =>
                        `  at ${f.functionName || '<anonymous>'} (${f.url}:${f.lineNumber}:${f.columnNumber})`,
                    )
        ...
        ws.addEventListener('error', () =>
          recordFailure('WebSocket connection failed'),
        );
        ws.addEventListener('close', () => {
          if (!monitoringEnded)
            recordFailure('WebSocket closed before monitoring completed');
        });

        setTimeout(() => {
          monitoringEnded = true;
          if (pending.size > 0)
            recordFailure('CDP subscriptions were not confirmed');
          try {
            ws.close();
          } catch {
            // ignore close error
      ...
      setTimeout(() => {
        ...
        if (
          ...
          confirmedTargets === selectedTargets.length
        ) {
          console.log(
            `0 errors/warnings captured across ${selectedTargets.length} target(s) during ${durationMs}ms window.`,
          );
        } else {
          process.exitCode = 1;
