...
test('Collapse Icons & Titles Characterization Suite', async (t) => {
  ...
  await t.test(
    'gbDonationTables render collapse icon, close button, and clickable title',
    () => {
      const {
        gbTabSafe,
        gbTabNotSafe,
      ...
      assert.match(
        safeHtml,
        /id="donationTextLabel"[^>]*data-bs-toggle="collapse"/,
      );
      assert.match(safeHtml, /id="donationicon"[^>]*data-bs-toggle="collapse"/);
      assert.match(safeHtml, /class="btn-close"/);
      assert.match(safeHtml, /id="donationText"[^>]*class="collapse show"/);

      const notSafeHtml = gbTabNotSafe(
        1,
        190,
        100,
        200,
      ...
      assert.match(
        notSafeHtml,
        /id="donationicon"[^>]*data-bs-toggle="collapse"/,
      );
      assert.match(notSafeHtml, /class="btn-close"/);
      assert.match(notSafeHtml, /id="donationText"[^>]*class="collapse show"/);

      const emptyHtml = gbTabEmpty({
        playerName: 'Tester',
        playerId: 123,
        gbData: { name: 'The Arc', level: 10 },
      });
      assert.match(
        emptyHtml,
        /id="donationTextLabel"[^>]*data-bs-toggle="collapse"/,
      ...
      assert.match(emptyHtml, /class="btn-close"/);
      assert.match(emptyHtml, /id="donationText"[^>]*class="collapse show"/);
    },
  );
});
