...
describe('GBG Signals & MessageDispatcher RPC Suite', () => {
  ...
  it('routes GuildBattlegroundService.getAction with action province_conquered to removeSignal', async () => {
    signalsState = [
      { provinceId: 15, signal: 'focus', id: 15 },
      { provinceId: 20, signal: 'focus', id: 20 },
    ];
    ...
    assert.equal(result.handled, true);
    assert.equal(signalsState.length, 1);
    assert.equal(signalsState[0].provinceId, 20);
    assert.equal(checkedProvincesCount, 1);
  });
});
