...
test('MetadataStore - Deduplication on Identical vs Changed Data & Proxy Aliases', () => {
  const store = new MetadataStore();
  const notifications = [];
  ...
  const entity = {
    id: 'X_SpaceAgeMars_Landmark2',
    name: 'Space Landmark',
    width: 4,
    length: 4,
  };

  // 1. Register for the first time -> notifies
  store.registerEntity(entity);
  assert.equal(notifications.length, 1);
  assert.equal(notifications[0].id, 'X_SpaceAgeMars_Landmark2');

  // 2. Register identical entity second time -> deduped, does NOT notify
  store.registerEntity({ ...entity });
  assert.equal(
    notifications.length,
    1,
    'Identical entity registration should not re-notify',
  );

  // 3. Register changed entity -> notifies again
  store.registerEntity({ ...entity, name: 'Space Landmark - Upgraded' });
  assert.equal(
    notifications.length,
    2,
    'Changed entity registration should notify',
  );
  assert.equal(notifications[1].entity.name, 'Space Landmark - Upgraded');

  // 4. Legacy CityEntityDefs Proxy - setting alias keys does NOT fire duplicate notifications
  const proxy = store.createLegacyCityEntityProxy();
  const initialNotificationCount = notifications.length;

  // Setting canonical key when already identical -> 0 new notifications
  proxy['X_SpaceAgeMars_Landmark2'] = {
    ...entity,
    name: 'Space Landmark - Upgraded',
  };
  assert.equal(notifications.length, initialNotificationCount);

  // Setting alias key -> does NOT trigger duplicate notifications
  proxy['SpaceAgeMars_Landmark2'] = {
    ...entity,
    name: 'Space Landmark - Upgraded',
  };
  proxy['building_entity_SpaceAgeMars_Landmark2'] = {
    ...entity,
    name: 'Space Landmark - Upgraded',
  };
  assert.equal(
    ...
    'Proxy alias writes must not re-notify',
  );
  assert.equal(
    store.getEntity('SpaceAgeMars_Landmark2')?.name,
    'Space Landmark - Upgraded',
  );
});
